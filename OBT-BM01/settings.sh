...
BRANCH_ID=master
# If there will be another partisan branch uncomment/update this part
#if [[ $(( $WEEK_DAY % 2 )) -eq 0 ]]
#then
...
# Local log archive
LOG_ARCHIEVE_DIR_EXPIRE=31 # days
...
SUPRESS_PLUGINS=' -D MAKE_CASSANDRAEMBED=1 -D SUPPRESS_COUCHBASEEMBED=ON -D USE_AZURE=OFF -D USE_AWS=OFF -D SUPPRESS_MYSQLEMBED=ON'
...
PERF_SUPRESS_PLUGINS=' -D INCLUDE_PLUGINS=0 -D MAKE_CASSANDRAEMBED=1 -D SUPPRESS_COUCHBASEEMBED=ON -D USE_AZURE=OFF -D USE_AWS=OFF -D SUPPRESS_MYSQLEMBED=ON'

PERF_SAR_MONITOR_START=1
PERF_MPSTAT_MONITOR_START=1
