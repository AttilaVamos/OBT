    ...
    def readData(self):
        def processLine(line):
            ...
            if testNameLen < 3:
                ...
                testDay = testNameItems[testNameLen-2]
                value = float(items[1])
...
#                continue
            ...
            for line in file:
                ...
                if testname not in self.results[cluster] :
                    self.results[cluster][testname]={'Dates' : [], 'Values' :[],  'Values2' : numpy.array([]),  'avg' : 0.0,  'sigma' : 0.0, 'runsPerDay':[],  'max': [],  'min': [], 'loops' : [] }
...
#                self.numOfRuns[cluster] = len(self.results[cluster][testname]['Dates'])
        ...
        for cluster in sorted(self.clusterTrends):
            ...
            for testname in sorted(self.results[cluster]):
                for day in sorted(self.results[cluster][testname]['Dates']):
        ...
        for cluster in sorted(self.clusterTrends):
            ...
            for testname in sorted(self.results[cluster]):
                numOfTestDays = len(self.results[cluster][testname]['Dates'])
                for dayIndex in range(numOfTestDays):
                    date = self.results[cluster][testname]['Dates'][dayIndex]
                    ...
                    #       Some execution missing, use some approximation: test is invalid , keep original value
                    #
                    value = self.results[cluster][testname]['Values'][dayIndex]
                    testLoopCount = self.results[cluster][testname]['runsPerDay'][dayIndex]
                    if testLoopCount > 1:
                        ...
                        self.results[cluster][testname]['Values'][dayIndex] = value
        ...
        for cluster in sorted(self.results):
            ...
            for test in sorted(self.results[cluster]):
                self.results[cluster][test]['avg'] = self.results[cluster][test]['Values2'].mean()
                self.results[cluster][test]['sigma'] = self.results[cluster][test]['Values2'].std()
                self.results[cluster][test]['all'] = self.CalcTrend(self.results[cluster][test]['Values'])
                
                dataPoints = len(self.results[cluster][test]['Values'])
                
                resultFile.write(test+','+ str(self.results[cluster][test]['avg']) + ',' + str(self.results[cluster][test]['sigma']) + ',' + str(self.results[cluster][test]['all']['alpha']) + ',' + str(self.results[cluster][test]['all']['beta']) + ',' + str(dataPoints))
                for index in range(dataPoints):
                    resultFile.write(',' + str(self.results[cluster][test]['Values'][index]))
                resultFile.write('\n')
            resultFile.close()
            pass
...
#            print("cluster:%s, test: %s" % (cluster,  test))
            ...
            self.myPrint("\t\t",  self.results[cluster][test])
            
            dataPoints = min(len(self.results[cluster][test]['Dates']), self.maxDatapoints)
            dates = self.results[cluster][test]['Dates'][-dataPoints:]
            
            for index in range(len(dates)):
                if len(dates[index]) == 6:
            ...
            self.results[cluster][test]['avg'] = self.results[cluster][test]['Values2'][-dataPoints:].mean()
            self.results[cluster][test]['sigma'] = self.results[cluster][test]['Values2'][-dataPoints:].std()
            self.results[cluster][test]['maxDataPoints']  = self.CalcTrend(self.results[cluster][test]['Values'][-dataPoints:])            
       
       
            # Plot the data
            ax.plot_date(dates2, self.results[cluster][test]['Values'][-dataPoints:], label=cluster, linestyle = '--', color=diagramColors['value'])
            #ax.plot_date(dates2, self.results[cluster][test]['Values'][-dataPoints:], linestyle = '--', color=diagramColors['value'])

            # plot the mean
            if self.enableMean:
                clusterTrendLabel = '%s (mean:%0.2f, sigma:%02f)' % ( cluster, self.results[cluster][test]['avg'], self.results[cluster][test]['sigma'])
                mx = [dates2[0],  dates2[-1]]
                my = [self.results[cluster][test]['avg'] ,  self.results[cluster][test]['avg'] ]
                ax.plot(mx, my, label=clusterTrendLabel, marker ='.', linestyle = '-', color=diagramColors['mean'] )
            
            # plot the sigma
            if self.enableSigma:
                mx = [dates2[0],  dates2[-1]]
                my = [self.results[cluster][test]['avg']+self.results[cluster][test]['sigma'], self.results[cluster][test]['avg']+self.results[cluster][test]['sigma']]
            ...
            # plot the trend
            if self.enableTrend:
                clusterTrendLabel = '%s-trend (%0.2f sec/day, alpha:%0.3f, beta:%0.3f)' % ( cluster, self.results[cluster][test]['maxDataPoints']['alpha'], self.results[cluster][test]['maxDataPoints']['alpha'], self.results[cluster][test]['maxDataPoints']['beta'])
                x1 = [dates2[0],  dates2[-1]]
                y1 = [self.results[cluster][test]['maxDataPoints']['beta'], self.results[cluster][test]['maxDataPoints']['alpha'] * (len(dates2) - 1) + self.results[cluster][test]['maxDataPoints']['beta'] ]
            ...
            if not self.disableMovingAverage1:
                ...
                movingAverages = self.calcMovingAverage(self.movingAverageWindow1, self.results[cluster][test]['Values'][-dataPoints:])
            ...
            if not self.disableMovingAverage2:
                ...
                movingAverages = self.calcMovingAverage(self.movingAverageWindow2, self.results[cluster][test]['Values'][-dataPoints:])
                ax.plot_date(dates2, movingAverages, label=movingAverage2Label, marker ='.', linestyle = '-', color=diagramColors['movingAverage2'])

            # Plot the min values
            if self.enableMin:
                ax.plot_date(dates2, self.results[cluster][test]['min'][-dataPoints:], label='Min', marker ='2', linestyle = '--', color=diagramColors['min'])
                
            # Plot the max values
            if self.enableMax:
                ax.plot_date(dates2, self.results[cluster][test]['max'][-dataPoints:], label='Max', marker ='1', linestyle = '--', color=diagramColors['max'])
            ...
            if self.enableLoopValues:
                ...
                for loop in (1, 2):
                    values = [ d[loop] if loop in d else 0 for d in self.results[cluster][test]['loops'][-dataPoints:] ]
            ...
            myTitle = tag + ' ' + wrapTestName + ' on ' + cluster + ' in last ' + str(dataPoints) +' days'
    ...
    def processResults(self):
        ...
        for cluster in sorted(self.results):
            ...
            for test in sorted(self.results[cluster]):
                ...
                lastDataDate = dt.date(2000+int(lastDataDateStr[0:2]), int(lastDataDateStr[2:4]), int(lastDataDateStr[4:6]))
...
#                        break
                    ...
                    if (self.results[cluster][test]['all']['alpha'] > self.badThreshold / 100.0) and (fluctuation >= 1.0):
                        print("test:%s\n\t\tmean:%f sec, sigma:%f sec, fluctuation:%f, alpha:%f" % (test, self.results[cluster][test]['avg'], self.results[cluster][test]['sigma'], fluctuation, self.results[cluster][test]['all']['alpha']))
                        numOfUglyBadTests += 1
                        averageTimeOfUglyBadTests += self.results[cluster][test]['avg']
                        if plt:
                            self.manageTestCase(cluster, test, self.uglyAndBadTag)
                            
                    elif self.results[cluster][test]['all']['alpha'] > self.badThreshold / 100.0:
                        print("test:%s\n\t\tmean:%f sec, sigma:%f sec, fluctuation:%f, alpha:%f" % (test, self.results[cluster][test]['avg'], self.results[cluster][test]['sigma'], fluctuation, self.results[cluster][test]['all']['alpha']))
                        numOfBadTests += 1
                        averageTimeOfBadTests += self.results[cluster][test]['avg']
                        if plt:
                        ...
                        print("test:%s\n\t\tmean:%f sec, sigma:%f sec, fluctuation:%f, alpha:%f" % (test, self.results[cluster][test]['avg'], self.results[cluster][test]['sigma'], fluctuation, self.results[cluster][test]['all']['alpha']))
                        numOfFluctTests += 1
                        averageTimeOfFlucTests += self.results[cluster][test]['avg']
                        if plt:
...
#                        pass
                    elif self.results[cluster][test]['all']['alpha'] < self.goodThreshold / 100.0:
                        print("test:%s\n\t\tmean:%f sec, sigma:%f sec, fluctuation:%f, alpha:%f" % (test, self.results[cluster][test]['avg'], self.results[cluster][test]['sigma'], fluctuation, self.results[cluster][test]['all']['alpha']))
                    ...
                    reportFile.write(",%f,%f,%f\n" % (self.results[cluster][test]['avg'], self.results[cluster][test]['sigma'], fluctuation))
                    
                    
                    if self.enablePdfReport:
                        rowData = [ self.separateAndWrapTestName(test), 
                                    "%0.2f" % self.results[cluster][test]['twoDays']['alpha'], self.results[cluster][test]['twoDays']['direction'],  "%0.2f" % self.results[cluster][test]['twoDays']['percentage'], 
                                    "%0.2f" % self.results[cluster][test]['fiveDays']['alpha'], self.results[cluster][test]['fiveDays']['direction'], "%0.2f" %  self.results[cluster][test]['fiveDays']['percentage'], 
                                    "%0.2f" % self.results[cluster][test]['all']['alpha'], self.results[cluster][test]['all']['direction'],  "%0.2f" % self.results[cluster][test]['all']['percentage']  
                            ]
...
#                    continue
                ...
                dataPoints = len(dates2)
                ax.plot_date(dates2, self.clusterTrends[cluster]['Totals'][-dataPoints:], label=cluster, linestyle = '--', color=clusterColors[cluster])
