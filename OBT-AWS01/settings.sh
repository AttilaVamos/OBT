...
SUPRESS_PLUGINS=' -DMAKE_CASSANDRAEMBED=1 -DSUPPRESS_COUCHBASEEMBED=ON -DUSE_AZURE=OFF -DUSE_AWS=OFF -DSUPPRESS_MYSQLEMBED=ON -DSUPPRESS_WASMEMBED=ON -DSKIP_ECLWATCH=ON'
...
REGRESSION_REPORT_SENDER="$USER<$OBT_ID>"
...
PERF_SKIP_VCPKG_HANDLING=0

PERF_CONTROL_TBB=0
PERF_USE_TBB=0
PERF_USE_TBBMALLOC=0

PERF_SUPRESS_PLUGINS=' -D INCLUDE_PLUGINS=1 -D MAKE_CASSANDRAEMBED=1 -D SUPPRESS_COUCHBASEEMBED=ON -D SUPPRESS_MYSQLEMBED=ON -D USE_MYSQL=OFF -D USE_AZURE=OFF -D USE_AWS=OFF -DSUPPRESS_WASMEMBED=ON -DSKIP_ECLWATCH=ON'
...
PERF_CALCTREND_PARAMS="--enableTestPlotGeneration --enableBaseLine --baseLineDate 2026-03-02"
