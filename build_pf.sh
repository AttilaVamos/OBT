...
CMAKE_CMD+=$' -DINCLUDE_PLUGINS='${TEST_PLUGINS}' -DTEST_PLUGINS='${TEST_PLUGINS}   # '-D TEST_PLUGINS' not used
...
CMAKE_CMD+=$' -DECLWATCH_BUILD_STRATEGY='${ECLWATCH_BUILD_STRATEGY}     # not used

CMAKE_CMD+=$' -DINCLUDE_SPARK='${ENABLE_SPARK}' -DSUPPRESS_SPARK='${SUPPRESS_SPARK}' -DSPARK='${ENABLE_SPARK}   # whole spark is not used

CMAKE_CMD+=$' -DUSE_MYSQL=OFF -DUSE_MYSQLEMBED=OFF -DSUPPRESS_MYSQLEMBED=ON'   # '-DUSE_MYSQLEMBED' not used

CMAKE_CMD+=$' '${PYTHON_PLUGIN}   # Need to revise related to Python2 stuff

CMAKE_CMD+=$' -DVCPKG_ECLBLAS_DYNAMIC_ARCH=OFF'

CMAKE_CMD+=$' -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DUSE_LIBXSLT=ON -DXALAN_LIBRARIES= -DUSE_NATIVE_LIBRARIES=ON'

# Outdated, should remove
