...
# Parameters: -d ~/shared/OBT-AWS01/Perfstat/ -v -r ~/tmp/graphsVM/  --enableTestPlotGeneration
#
# For generate base line, add: --generateBaseLine
# For generate base line on a specific day: add:  --baseLineDate "2025-04-22"
#
# To use base line in generating diagrams, add: --enableBaseLine --baseLineDate "2025-04-22"
# if '--baseLineDate "2025-04-22"'
#    - is not present,
#    - or not found a baseline file corresponding the date,
#    - or any proble occurs during procesing the file
# then the error message displayed, basline disabled and the process continues.
#
    ...
    def __init__(self, options):
        ...
        if self.enableBaseLine:
            if self.baseLineDate == "":
                print("Error: Using base line is enabled ('--enableBaseLine'), but missing base line date ('--baseLineDate <YYYY-MM-DD>').")
...
#                    continue
    ...
    def manageTestCase(self, cluster, test, tag, testData: dict):
        ...
        if self.enableTestPlotGeneration and False:
            self.createPlot(cluster, test, tag,  testData)

    @staticmethod
    def createPlot(self,  cluster, test,  tag,  testData: dict):
        self.myPrint("cluster:%s, test:%s, tag:%s" % (cluster, test, tag))
        diagramFileName =''
        try:
            print("cluster:%s, test: %s" % (cluster,  test))
#            if self.useAllData == True:
#                dataPoints = len(testData['Days'])
#                dataPoints = len(self.results2[cluster][test]['Days'])
#            else:
#                dataPoints = min(len(testData['Days']), self.maxDatapoints)
#                dataPoints = min(len(self.results2[cluster][test]['Days']), self.maxDatapoints)

            dataPoints = testData['dataPoints']
            self.myPrint("\t\t",  testData)
            
            dates = testData['Dates'][-dataPoints:]
...
#            try:
#                testData['avg'] = numpy.nanmean( testData['Values2'][-dataPoints:])
#                testData['sigma'] = numpy.nanstd( testData['Values2'][-dataPoints:])
#                
...
#                if self.verbose:
#                    PrintException(repr(e) + " Problem with an older numpy.")
#                # A hack for an older numpy where nonmean() and nanstd() doesn't exists
#                v2 = testData['Values2'][~numpy.isnan(testData['Values2'])][-dataPoints:]
#                testData['avg'] = numpy.mean(v2)
#                testData['sigma'] = numpy.std(v2)
#                
#                v2 = self.results2[cluster][test]['Values2'][~numpy.isnan(self.results2[cluster][test]['Values2'])][-dataPoints:]
#                self.results2[cluster][test]['avg'] = numpy.mean(v2)
#                self.results2[cluster][test]['sigma'] = numpy.std(v2)
#                    
#            testData['maxDataPoints']  = self.CalcTrend(testData['Values2'][-dataPoints:])            
#            self.results2[cluster][test]['maxDataPoints']  = self.CalcTrend(self.results2[cluster][test]['Values2'][-dataPoints:])            
       
       
            # Plot the data
            #ax.plot_date(dates2, testData['Values2'][-dataPoints:], label=cluster, linestyle = '--', color=diagramColors['value'])
        ...
        for cluster in sorted(self.results2):
            ...
            for test in sorted(self.results2[cluster]):
                ...
                if debug:
                    ...
                    if (not test.startswith('03da')) and (not test.startswith('15b')) :
                        continue
                ...
                if dataPoints > 1:
                    ...
                    if self.useAllData == True:
                        self.results2[cluster][test]['dataPoints'] = len(self.results2[cluster][test]['Days'])
                    else:
                        self.results2[cluster][test]['dataPoints'] = min(len(self.results2[cluster][test]['Days']), self.maxDatapoints)
...
#                        break
                    ...
                    if (self.results2[cluster][test]['all']['alpha'] > self.badThreshold / 100.0) and (fluctuation >= 1.0):
                        ...
                        tag =  self.uglyAndBadTag
                        if plt:
                            self.manageTestCase(cluster, test, self.uglyAndBadTag, self.results2[cluster][test])
                        ...
                        tag =  self.badTag
                        if plt:
                            self.manageTestCase(cluster,  test,  self.badTag, self.results2[cluster][test])
                        ...
                        tag =  self.uglyTag
                        if plt:
                            self.manageTestCase(cluster,  test,  self.uglyTag, self.results2[cluster][test])
...
#                        pass
                        ...
                        if plt and self.enableReportGood:
                            self.manageTestCase(cluster,  test,  self.goodTag, self.results2[cluster][test])
                    elif enableGood:
                        print("\t\tmean:%f sec, sigma:%f sec, fluctuation:%f, alpha:%f" % (self.results2[cluster][test]['avg'], self.results2[cluster][test]['sigma'], fluctuation, self.results2[cluster][test]['all']['alpha']))
                        if plt and self.enableReportGood:
                            self.manageTestCase(cluster,  test, self.neutralTag, self.results2[cluster][test])

                    tasks.append((cluster,  test,  tag, self.results2[cluster][test] ))
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor(max_workers=1) as executor:
        executor.map(tr.createPlot, tasks,  chunksize=4)
